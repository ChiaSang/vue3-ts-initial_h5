...
import { LoadingInterface, ToastInterface, errorFormatter, wechatLogin, removeEmpty } from '@/utils/util'
...
axiosInstance.interceptors.response.use(
  async (response: AxiosResponse<ResponseData>) => {
    ...
    const resultCode = response?.data?.result?.resultCode.toString()
    // 2xx 范围内的状态码都会触发该函数。
    // 对响应数据做点什么
    if (config.retried) {
