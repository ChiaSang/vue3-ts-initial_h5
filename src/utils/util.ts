...
export const errorFormatter = (error: any, methodName = '') => {
  ...
  if (error?.isAxiosError) {
    ...
    console.error('AxiosError toJSON', error?.toJSON())
 ...
 */
export function wechatLogin(scope = 'snsapi_base'): void {
  const params = {
  ...
  const url = `https://open.weixin.qq.com/connect/oauth2/authorize?${qs.stringify(params)}#wechat_redirect`
